...
const surfaceCardClass =
  'product-surface rounded-[1.35rem] border border-white/55 bg-white/70 shadow-[0_24px_90px_rgba(236,72,153,0.16)] dark:border-white/10 dark:bg-white/[0.07]';
  ...
  if (!userProfile) {
    return (
        <div className={`min-h-screen ${isDarkMode ? 'dark' : ''} overflow-x-hidden bg-[var(--background)] text-slate-900 dark:text-white`}>
          ...
          <nav className="fixed top-0 z-50 w-full border-b border-white/50 bg-white/55 backdrop-blur-2xl dark:border-white/10 dark:bg-[#120914]/60">
            <div className="max-w-7xl mx-auto px-4 md:px-6 h-[4.5rem] md:h-20 flex justify-between items-center">
              ...
              <div className="flex items-center gap-2 md:gap-4">
                ...
                <button onClick={() => openAuthModal('register')} className="px-4 md:px-8 py-2.5 md:py-3 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-xl font-semibold text-xs md:text-sm shadow-[0_14px_32px_rgba(236,72,153,0.34)] hover:shadow-[0_18px_40px_rgba(236,72,153,0.42)] active:translate-y-px transition-all">Старт</button>
                <button onClick={() => setIsDarkMode(!isDarkMode)} className="rounded-xl border border-white/60 bg-white/60 p-2.5 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/8 md:p-3">{isDarkMode ? <Sun size={18}/> : <Moon size={18}/>}</button>
              </div>
            </div>
          </nav>
          ...
          <main className="relative pt-28 md:pt-32 pb-24 px-4 md:px-6 max-w-7xl mx-auto space-y-12">
            ...
            <div className="grid min-h-[calc(100vh-9rem)] items-center gap-10 lg:grid-cols-[0.94fr_1.06fr]">
              <motion.div initial={{ opacity: 0, x: -32 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                <div className="inline-flex items-center gap-2 rounded-full border border-white/55 bg-white/58 px-5 py-2 text-[11px] font-semibold uppercase text-pink-600 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/8 dark:text-pink-200">
                  <Zap size={14} /> Стартап для сучасного вчителя
                </div>
                <div className="space-y-5">
                  <h1 className="max-w-3xl text-5xl font-extrabold leading-[0.9] sm:text-6xl md:text-8xl">
                    Твій <span className="soft-gradient-text">ШІ</span> <br /> асистент
                  </h1>
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  <button
                    ...
                    className="group inline-flex w-full items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-pink-500 to-rose-500 px-8 py-5 text-lg font-bold text-white shadow-[0_22px_45px_rgba(236,72,153,0.34)] transition-all duration-200 hover:shadow-[0_28px_58px_rgba(236,72,153,0.44)] active:translate-y-px sm:w-auto md:px-10 md:py-6 md:text-xl"
                  >
                    Почати роботу
                  ...
                  <button
                    ...
                    className="inline-flex w-full items-center justify-center rounded-2xl border border-white/65 bg-white/58 px-8 py-5 text-sm font-bold uppercase text-slate-700 shadow-sm backdrop-blur transition-all duration-200 hover:border-pink-500/40 dark:border-white/10 dark:bg-white/8 dark:text-white/80 sm:w-auto"
                  >
                    Увійти
                  </button>
                </div>
                <div className="grid max-w-2xl grid-cols-3 gap-3 pt-2">
                  {[
                    ...
                    <div key={value} className="rounded-2xl border border-white/65 bg-white/55 px-4 py-3 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/[0.06]">
              ...
              <motion.div initial={{ opacity: 0, y: 22, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="relative hidden lg:block">
                <div className={`${surfaceCardClass} relative overflow-hidden p-5 md:p-6`}>
                  ...
                  <div className="relative rounded-[1.25rem] border border-white/60 bg-white/50 p-4 backdrop-blur dark:border-white/10 dark:bg-[#160d1d]/65">
                    ...
                    <div className="grid gap-4">
                      <div className="rounded-[1.1rem] border border-white/65 bg-white/72 p-5 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/[0.07]">
                        <div className="flex items-center justify-between">
                          <div className="text-[11px] font-semibold uppercase text-pink-500">Тема</div>
                          <Clock3 size={15} className="text-slate-400" />
                        </div>
                        <div className="mt-3 text-2xl font-bold leading-tight text-slate-900 dark:text-white">Будова тексту та головна думка</div>
                        <div className="mt-4 grid grid-cols-3 gap-2">
                          {['Мета', 'Хід уроку', 'Рефлексія'].map((item) => (
                            <div key={item} className="rounded-xl bg-pink-500/10 px-3 py-2 text-center text-[11px] font-bold text-pink-600 dark:text-pink-200">{item}</div>
                          ))}
                        </div>
                      </div>
                      <div className="grid grid-cols-[0.82fr_1.18fr] gap-4">
                        <div className="rounded-[1.1rem] bg-gradient-to-br from-slate-950 to-pink-950 p-5 text-white dark:from-white dark:to-pink-100 dark:text-slate-950">
                        ...
                        <div className="rounded-[1.1rem] border border-white/65 bg-white/72 p-5 backdrop-blur dark:border-white/10 dark:bg-white/[0.07]">
                          <div className="mb-4 flex items-center justify-between">
                      ...
                      <div className="rounded-[1.1rem] border border-pink-500/18 bg-gradient-to-r from-pink-500/12 to-rose-500/10 p-4">
          ...
          <AnimatePresence>
            {showAuthModal && (
              <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/54 backdrop-blur-md">
                ...
                <motion.div 
                  ...
                  className="relative z-10 w-full max-w-[460px] rounded-[1.5rem] border border-white/60 bg-white/82 p-6 shadow-[0_28px_90px_rgba(236,72,153,0.22)] backdrop-blur-2xl dark:border-white/10 dark:bg-[#160d1d]/88 md:p-12"
                >
                  ...
                  <div className="relative my-10 text-center">
                    ...
                    <span className="relative px-6 bg-white dark:bg-[#160d1d] text-[11px] font-semibold uppercase opacity-35 tracking-wide">Або за допомогою</span>
                  </div>

                  <div className="mb-8 md:mb-10 flex justify-center overflow-hidden">
                    {GOOGLE_CLIENT_ID ? (
                      ...
                      <button
                        type="button"
                        disabled
                        className="inline-flex h-11 min-w-[220px] items-center justify-center gap-3 rounded-full border border-slate-200 bg-white px-5 text-sm font-semibold text-slate-400 shadow-sm dark:border-white/10 dark:bg-white/5 dark:text-white/35"
                        title="Додайте NEXT_PUBLIC_GOOGLE_CLIENT_ID, щоб увімкнути Google-вхід"
                      >
                        <span className="flex h-5 w-5 items-center justify-center rounded-full border border-slate-200 text-[12px] font-bold dark:border-white/15">G</span>
                        Увійти через Google
                      </button>
                    )}
                  </div>
